...
qui  gen De_`yF'=  _merge==3      // employed 
qui  gen De_same_`yF'= _merge==3 & fid==fidDEC`y' //  employed in same firm
qui  gen De_other`yF'= _merge==3 & fid!=fidDEC`y' //  employed in other firm

qui  gen Dmover_`yF' = fid==fidDEC`y' if _merge==3

qui  gen Dw_`yF' = (income - incomeDEC`y')/incomeDEC`y' if De_`yF'==1
qui  gen Dw_other_`yF' = (income - incomeDEC`y')/incomeDEC`y' if De_other`yF'==1
qui  gen Dw_same_`yF' = (income - incomeDEC`y')/incomeDEC`y'  if  De_same_`yF'==1
...
foreach v in e e_same w w_same w_other {
...
qui  gen De_same_corr_`i' = De_same_`i' - beta_e_same_2011 
qui replace De_same_corr_`i' = De_same_`i' if wbin==.

qui  gen Dw_same_corr_`i' = Dw_same_`i' - beta_w_same_2011 
qui replace Dw_same_corr_`i' = Dw_same_`i' if wbin==.

qui  gen Dw_other_corr_`i' = Dw_other_`i' - beta_w_other_2011 
qui replace Dw_other_corr_`i' = Dw_other_`i' if wbin==.

foreach v in e_corr_`i' e_same_corr_`i' w_corr_`i' w_same_corr_`i' w_other_corr_`i' {
...
tw (connect beta_w_corr_2013 wbin, lcolor("32 178 170") mcolor("32 178 170 %34")) (rcap cilow_w_corr_2013 cihigh_e_corr_2013 wbin, lpattern(solid) lcolor("32 178 170")) (connect beta_w_same_corr_2013 wbin, lpattern(solid) lcolor("251 162 127") mcolor("251 162 127 %34")  ) (rcap cilow_w_same_corr_2013 cihigh_w_same_corr_2013 wbin, lpattern(solid) lcolor("251 162 127")), xline(373, lcolor(red%85)) yline(0, lcolor(black%25)) xtitle("Monthly Income Bins") ytitle("Stay in Employment")  ylabel(-0.05(0.05)0.25) xlabel(289(28)1017, alternate)  legend(order( 1 "Any Employer" 3 "Same Employer" )) 

tw (connect beta_e_corr_2013 wbin, lcolor("32 178 170") mcolor("32 178 170 %34")) (rcap cilow_e_corr_2013 cihigh_e_corr_2013 wbin, lpattern(solid) lcolor("32 178 170")) (connect beta_e_same_corr_2013 wbin, lpattern(solid) lcolor("251 162 127") mcolor("251 162 127 %34")  ) (rcap cilow_e_same_corr_2013 cihigh_e_same_corr_2013 wbin, lpattern(solid) lcolor("251 162 127")), xline(373, lcolor(red%85)) yline(0, lcolor(black%25)) xtitle("Monthly Income Bins") ytitle("Stay in Employment")  ylabel(-0.08(0.02)0.04)  xlabel(289(28)1017, alternate)  legend(order( 1 "Any Employer" 3 "Same Employer" )) 
